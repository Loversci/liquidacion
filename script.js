document.addEventListener('DOMContentLoaded', function() {
    ...
    // CORRECCIÓN: El listener ahora apunta a la nueva función de impresión, que es más robusta.
    ...
    function calcularYMostrarLiquidacion() {
        // --- Toda la lógica de cálculo permanece igual, ya que es correcta ---
        const data = {
        ...
        const totalIngresosBrutos = sueldoPendienteMonto + aguinaldoProporcional + vacacionesMonto + indemnizacionMonto;
        const baseCalculoDeducciones = sueldoPendienteMonto + aguinaldoProporcional + vacacionesMonto;
        ...
        // CORRECCIÓN: Se actualiza el HTML para usar las nuevas clases de tabla en lugar de grids/flexbox para mejor impresión.
        pdfContent.innerHTML = `
            ...
            <b>INGRESOS</b><hr>
            <table class="prestaciones-table"><tr><td>SUELDO PENDIENTE DE PAGO</td><td>DEL</td><td>${formatDate(data.fechaInicioSueldoPendiente)}</td><td>AL DÍA</td><td>${formatDate(fechaFinSueldoPendiente)}</td><td>${data.sueldoPendienteDias}</td><td class="monto">${f(sueldoPendienteMonto)}</td></tr></table>
    ...
    // ==================================================================================
    // FUNCIÓN DEFINITIVA PARA IMPRIMIR / GENERAR PDF
    // Reemplaza la funcionalidad de html2pdf.js por una solución nativa y más confiable.
    // ==================================================================================
    function imprimirLiquidacion() {
        const contenidoParaImprimir = document.getElementById('pdf-content').innerHTML;
        // Obtenemos el enlace a la hoja de estilos para que la impresión se vea idéntica.
        const estilos = document.querySelector('link[href="style.css"]').outerHTML;

        // Abrimos una nueva ventana/pestaña en el navegador.
        const ventanaImpresion = window.open('', '_blank');

        // Escribimos el contenido y los estilos en la nueva ventana.
        ventanaImpresion.document.write(`
            <html>
                <head>
                    ...
                    ${estilos}
                    <style>
                        /* Estilos adicionales solo para la vista de impresión */
                        body { margin: 25px; background-color: #fff; }
                        @media print {
                           /* Ocultar elementos no deseados de la interfaz del navegador al imprimir */
                           body { margin: 0; }
                        }
                    </style>
                </head>
        ...
        ventanaImpresion.document.close(); // Necesario para que el navegador termine de cargar la página.

        // Esperamos a que todo el contenido (incluyendo imágenes y fuentes) se cargue.
        ventanaImpresion.onload = function() {
            ventanaImpresion.focus(); // Damos foco a la nueva ventana.
            ventanaImpresion.print(); // Abrimos el diálogo de impresión del navegador.
            ventanaImpresion.close(); // Cerramos la ventana después de que el usuario imprima o cancele.
        };
    }
